...
// README examples are a contract: every `cli ...` line inside a
// fenced bash block of README.md must be a real invocation. Each one runs
// in-process with --dry-run appended (the request is built and previewed,
// nothing is sent), so a renamed command, a removed flag or a leftover
// `<placeholder>` fails this test instead of rotting in the docs.
//
// Skipped on purpose:
...
//     completion, help, and any invocation passing --interactive
//   - blocks preceded by an HTML comment `<!-- readme-examples: skip ... -->`
//     (examples that need local files or a live service)
//
// Anything after an unquoted pipe or redirect (`| jq`, `> out.json`) is not
// part of the invocation and is dropped.
func TestReadmeExamples(t *testing.T) {
	readmePath := findReadme(t)
	examples := collectReadmeExamples(t, readmePath)
	require.NotEmpty(t, examples, "no runnable README examples found in %s", readmePath)
	...
	for _, ex := range examples {
		...
		t.Run(fmt.Sprintf("line_%d", ex.line), func(t *testing.T) {
			...
			var err error
			if ex.stdin != "" {
				err = h.RunWithStdin(args, ex.stdin)
			} else {
				err = h.Run(args)
			}
			require.NoError(t, err, "README.md:%d: %s\nstderr:\n%s", ex.line, ex.raw, h.GetStderr())
		})
	}
}
...
func parseReadmeInvocation(line string) (readmeExample, bool) {
	...
	if i >= len(words) || words[i].op != "" || words[i].text != "cli" {
		return readmeExample{}, false
	}
	i++
	for ; i < len(words); i++ {
	...
	for _, a := range ex.args {
		// An explicit --interactive request needs a terminal to prompt for
		// missing inputs; off-TTY a bare required-input command is a usage
		// failure by contract.
...
func TestParseReadmeInvocationAllowsQuotedEnvironmentValue(t *testing.T) {
	ex, ok := parseReadmeInvocation(`CLI_QUERY_PARAM1="value with spaces" cli version`)
	require.True(t, ok)
	require.Equal(t, map[string]string{"CLI_QUERY_PARAM1": "value with spaces"}, ex.env)
	require.Equal(t, []string{"version"}, ex.args)

	_, ok = parseReadmeInvocation(`"CLI_QUERY_PARAM1"=value cli version`)
	require.False(t, ok, "a quoted environment variable name is not an assignment")
}

func TestShellWordsPreservesNonSpecialBackslashInDoubleQuotes(t *testing.T) {
	words, ok := shellWords(`cli --jq "\d+\$"`)
	require.True(t, ok)
	require.Len(t, words, 3)
	require.Equal(t, `\d+$`, words[2].text)
}
