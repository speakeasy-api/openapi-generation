...
func ValidateOutputFormatFlag(cmd *cobra.Command, allowed []string) error {
	...
	case lookupFlag(cmd, "out") != nil:
		err.hint = fmt.Sprintf("-o sets the output format. To write to a file, use --out %s", val)
	case lookupFlag(cmd, "output-file") != nil:
		err.hint = fmt.Sprintf("-o sets the output format. To write the response body to a file, use --output-file %s", val)
	case looksLikePath(val):
		err.hint = fmt.Sprintf("-o sets the output format, not a file. To save the response: -o json > %s", val)
	}
	return err
}
...
func ShorthandConfusionHint(cmd *cobra.Command, args []string, errMsg string) string {
	for i, arg := range args {
		...
		related := (named != "" && strings.Contains(errMsg, "--"+named)) || strings.Contains(errMsg, global.Name) || (isBool && value != "")
		if !related {
			continue
		}
		if named == "" {
