...
func (c *clientCredentialsHook) getCredentialsGlobal(sec any) (*credentials, error) {
	...
	if security.Option6 == nil {
		return nil, nil
	}
	secType := reflect.TypeOf(security.Option6)
	if secType.Kind() == reflect.Ptr {
		secType = secType.Elem()
	}
	secValue := reflect.ValueOf(security.Option6)
	if secValue.Kind() == reflect.Ptr {
		secValue = secValue.Elem()
	}
	if security.Option6.TokenURL == "" {
		...
		security.Option6.TokenURL = tokenURLDefault
	}
	...
	return &credentials{
		ClientID:             security.Option6.ClientID,
		ClientSecret:         security.Option6.ClientSecret,
		TokenURL:             security.Option6.TokenURL,
		Scopes:               nil,
