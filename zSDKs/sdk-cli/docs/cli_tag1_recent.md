...
List recent test1 pages via tag1

```
cli tag1 recent [flags]
```
...
* `cli tag1 recent --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli tag1 recent --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
