...
```
cli test-group tag3 post-test [flags]
```
...
* [cli test-group tag3](cli_test-group_tag3.md)	 - Operations for tag3

### Machine interface

* `cli test-group tag3 post-test --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli test-group tag3 post-test --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli test-group tag3 post-test --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
