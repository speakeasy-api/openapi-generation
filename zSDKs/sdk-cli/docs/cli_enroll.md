...
* `cli enroll --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli enroll --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli enroll --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
