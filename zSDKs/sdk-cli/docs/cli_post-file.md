...
```
  cli post-file --upload <path/to/file>
```

### Options

```
  -h, --help                 help for post-file
  ...
  -u, --upload string        A file [required]
```
...
* `cli post-file --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli post-file --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
