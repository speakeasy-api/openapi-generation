...
```
  cli update-user --id <id> --user.id 8ffac18c-7d88-4879-b057-e5f45b9ce7de --user.email Joanny.Feeney@gmail.com
```
...
```
  ...
  -i, --id string                         [required]
      ...
      --user.first-name string            string value
      --user.gender string                options: male, female, other
      --user.id string                    [required]
      --user.last-name string             string value
      --user.metadata string              JSON object
      --user.metadata-any string          A metadata object with additionalProperties true (any type) - should be flattened
      --user.postal-code string           string value
```
...
* `cli update-user --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli update-user --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli update-user --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
