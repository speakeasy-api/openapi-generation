...
```
  cli test-endpoint --test-name <value> --test <value>
```
...
```
  ...
  -h, --help               help for test-endpoint
      ...
      --test-name string   string value (or pass it as the [test-name] argument)
```
...
* `cli test-endpoint --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli test-endpoint --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli test-endpoint --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
