...
Create a user record from an email address. Full request control:
"cli create-user".
...
```
  ...
  -g, --gender string                options: male, female, other
      --given-name string            First name to record
  ...
  -p, --postal-code string           string value
      ...
      --user-gender string           Gender to record (e.g. male, female, other)
```
...
* `cli invite --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli invite --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli invite --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
