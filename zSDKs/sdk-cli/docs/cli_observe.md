...
Produce an asset and print its terminal status
...
* `cli observe --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli observe --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli observe --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
