...
```
  cli chat --request '{"model":"review-model","prompt":"What is the largest city in the world?","stream":false}'
```
...
```
  -h, --help                help for chat
      ...
      --request string      Request body as JSON [required]. Can also be provided via stdin; @path reads a file, @- reads stdin to EOF.
...
* `cli chat --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli chat --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli chat --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
