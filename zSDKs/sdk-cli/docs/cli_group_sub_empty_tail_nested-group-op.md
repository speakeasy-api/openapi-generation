...
```
cli group sub empty tail nested-group-op [flags]
```
...
```
  -h, --help   help for nested-group-op
```
...
* [cli group sub empty tail](cli_group_sub_empty_tail.md)	 - Operations for tail

### Machine interface

* `cli group sub empty tail nested-group-op --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli group sub empty tail nested-group-op --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
