## cli tag1 post-file-with-encoding
...
```
cli tag1 post-file-with-encoding [flags]
```

### Examples

```
  cli tag1 post-file-with-encoding --file <path/to/file>
```

### Options

```
  -a, --attachment string     An optional binary attachment
      --file string           The file to upload (supports CSV, PNG, JPEG, or PDF) [required]
      --file-name string      Optional custom file name
      --file-purpose string   Purpose of the file upload
  -h, --help                  help for post-file-with-encoding
  -m, --metadata string       JSON metadata about the file
```
...
* `cli tag1 post-file-with-encoding --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli tag1 post-file-with-encoding --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
