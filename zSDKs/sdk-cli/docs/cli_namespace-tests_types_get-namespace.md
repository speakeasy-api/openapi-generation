...
```
cli namespace-tests types get-namespace [flags]
```
...
* `cli namespace-tests types get-namespace --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli namespace-tests types get-namespace --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
