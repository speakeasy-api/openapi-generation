...
```
cli get-named-primitive-union [flags]
```
...
```
  -h, --help   help for get-named-primitive-union
```
...
* `cli get-named-primitive-union --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli get-named-primitive-union --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
