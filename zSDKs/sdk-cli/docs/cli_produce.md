...
Produce an image asset and wait for completion

### Synopsis

Start an asset job, poll it to a terminal state, and write the image
...
* `cli produce --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli produce --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli produce --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
