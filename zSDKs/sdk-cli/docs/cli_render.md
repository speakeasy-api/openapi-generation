...
Render an image asset to a file

### Synopsis

Render an image from a text prompt and write it to a file; stdout
...
* `cli render --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli render --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli render --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
