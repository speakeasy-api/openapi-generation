## cli get-fully-flattened-request

Get Fully Flattened Request
...
```
cli get-fully-flattened-request [flags]
```

### Examples

```
  cli get-fully-flattened-request --lang en --name <value>
```
...
```
  ...
  -e, --emoji string     options: wave, smiley, thumbsup, tada
  -g, --gif string       string value
  -h, --help             help for get-fully-flattened-request
  -l, --lang string      [required]
  -m, --max-length int   integer value
  -n, --name string      [required]
  -p, --private          boolean flag
  -r, --reply            boolean flag
...
* `cli get-fully-flattened-request --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli get-fully-flattened-request --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli get-fully-flattened-request --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
