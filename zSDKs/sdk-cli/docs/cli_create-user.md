...
```
cli create-user [flags]
```

### Examples

```
  cli create-user --email request-only@example.com
```
...
* `cli create-user --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli create-user --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli create-user --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
