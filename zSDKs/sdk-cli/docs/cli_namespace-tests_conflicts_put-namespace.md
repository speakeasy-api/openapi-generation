...
```
cli namespace-tests conflicts put-namespace [flags]
```
...
```
  ...
  -h, --help                help for put-namespace
  -n, --namespaced string   JSON object
...
* `cli namespace-tests conflicts put-namespace --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli namespace-tests conflicts put-namespace --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli namespace-tests conflicts put-namespace --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
