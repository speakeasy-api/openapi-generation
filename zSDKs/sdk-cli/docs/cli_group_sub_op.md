...
* [cli group sub](cli_group_sub.md)	 - Operations for sub-group

### Machine interface

* `cli group sub op --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli group sub op --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
