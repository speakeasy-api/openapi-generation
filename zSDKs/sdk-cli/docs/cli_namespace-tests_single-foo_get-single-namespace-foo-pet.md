...
```
cli namespace-tests single-foo get-single-namespace-foo-pet [flags]
```
...
```
  -h, --help   help for get-single-namespace-foo-pet
```
...
* `cli namespace-tests single-foo get-single-namespace-foo-pet --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli namespace-tests single-foo get-single-namespace-foo-pet --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
