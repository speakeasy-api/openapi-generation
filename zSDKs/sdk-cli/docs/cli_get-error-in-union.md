...
```
  -h, --help   help for get-error-in-union
```
...
* `cli get-error-in-union --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli get-error-in-union --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
