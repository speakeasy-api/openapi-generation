...
```
cli render-asset [flags]
```

### Examples

```
  cli render-asset --prompt <value>
```
...
* `cli render-asset --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli render-asset --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli render-asset --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
