...
```
cli create-with-union [flags]
```

### Examples

```
  cli create-with-union --name <value> --shape '{"type":"rectangle","width":922.51,"height":3125.73}'
```
...
```
      ...
      --dry-run-param                        If true, validates without creating
  -h, --help                                 help for create-with-union
  -n, --name string                          Name for the shape [required]
      --schema                               Print the exact JSON Schema of the request body and exit
  -s, --shape string                         JSON value (variants: circle: { "type": string, "radius": number, ... }, rectangle: { "type": string, "width": number, "height": number, ... })
      --shape.circle string                  Circle variant as JSON
      --shape.circle.color-param string      Optional color of the circle
      --shape.circle.radius float            The radius of the circle [required]
      --shape.rectangle string               Rectangle variant as JSON
      --shape.rectangle.color-param string   Optional color of the rectangle
      --shape.rectangle.height float         The height of the rectangle [required]
      --shape.rectangle.width float          The width of the rectangle [required]
```
...
* `cli create-with-union --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli create-with-union --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli create-with-union --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
