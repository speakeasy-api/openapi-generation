## cli binary-and-string-upload

Binary And String Upload
...
```
cli binary-and-string-upload [flags]
```
...
```
  -b, --binary string   binary data (file:<path> or b64:<base64>)
  ...
  -h, --help            help for binary-and-string-upload
      --schema          Print the exact JSON Schema of the request body and exit
  -s, --string string   string value
```
...
* `cli binary-and-string-upload --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli binary-and-string-upload --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli binary-and-string-upload --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
