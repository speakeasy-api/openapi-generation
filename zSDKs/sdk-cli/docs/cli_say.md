...
Send a prompt and print the reply as it streams. Full request
control: "cli chat".
...
* `cli say --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli say --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli say --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
