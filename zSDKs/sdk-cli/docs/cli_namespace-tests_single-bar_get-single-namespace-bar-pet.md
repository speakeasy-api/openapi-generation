...
```
cli namespace-tests single-bar get-single-namespace-bar-pet [flags]
```
...
```
  -h, --help   help for get-single-namespace-bar-pet
```
...
* [cli namespace-tests single-bar](cli_namespace-tests_single-bar.md)	 - Operations for single-bar

### Machine interface

* `cli namespace-tests single-bar get-single-namespace-bar-pet --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli namespace-tests single-bar get-single-namespace-bar-pet --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
