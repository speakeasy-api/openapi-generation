## cli operation-with-leading-and-trailing-underscores

Operation With Leading And Trailing Underscores
...
```
cli operation-with-leading-and-trailing-underscores [flags]
```

### Examples

```
  cli operation-with-leading-and-trailing-underscores --qp1 renamed
```

### Options

```
  -h, --help         help for operation-with-leading-and-trailing-underscores
      --qp1 string   This parameter will not be filled in with the queryParam1 global because it uses x-speakeasy-name-override which results in a non-matching name. [required]
```
...
* `cli operation-with-leading-and-trailing-underscores --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli operation-with-leading-and-trailing-underscores --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
