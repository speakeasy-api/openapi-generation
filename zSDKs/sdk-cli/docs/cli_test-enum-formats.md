...
```
cli test-enum-formats [flags]
```

### Examples

```
  cli test-enum-formats --string-array-format pending_review --string-map-format medium_priority --string-partial-map-format draft_mode --integer-map-format 200 --integer-partial-map-format 1
```
...
```
  ...
  -h, --help                                help for test-enum-formats
      --integer-map-format string           Integer enum with x-speakeasy-enums as map (full coverage) (options: 100, 200, 300, 400) [required]
      --integer-partial-map-format string   Integer enum with x-speakeasy-enums as map (partial coverage) (options: 1, 2, 3, 4, 5) [required]
      --schema                              Print the exact JSON Schema of the request body and exit
      --string-array-format string          String enum with x-speakeasy-enums as array (full coverage) (options: pending_review, approved_final, rejected_permanently, arch_x86, mobile_ios, custom_field) [required]
      --string-map-format string            String enum with x-speakeasy-enums as map (full coverage) (options: low_priority, medium_priority, high_priority, critical_urgent) [required]
      --string-partial-map-format string    String enum with x-speakeasy-enums as map (partial coverage) (options: draft_mode, under_review, published_live, archived_old) [required]
```
...
* [cli](cli.md)	 - SDK Review: A test document for reviewing the SDK.

### Machine interface

* `cli test-enum-formats --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli test-enum-formats --schema` — the exact JSON Schema of the request body (all `$ref`s bundled)
* `cli test-enum-formats --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
