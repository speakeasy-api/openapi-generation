...
```
cli get-error-only-example [flags]
```
...
```
  -h, --help   help for get-error-only-example
```
...
* `cli get-error-only-example --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli get-error-only-example --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
