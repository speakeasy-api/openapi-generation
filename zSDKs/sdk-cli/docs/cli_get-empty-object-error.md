...
```
cli get-empty-object-error [flags]
```
...
```
  -h, --help   help for get-empty-object-error
```
...
* `cli get-empty-object-error --usage` — this command's flags, defaults and env vars as machine-readable KDL
* `cli get-empty-object-error --dry-run` — preview the request without OS-keychain access or a network call (human preview on stderr)
