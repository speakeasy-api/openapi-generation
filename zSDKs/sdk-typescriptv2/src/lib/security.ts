    ...
    [
      {
        type: "http:basic",
        value: {
          username: security?.userPassAuth?.username
            ?? env().SPEAKEASY_USERNAME,
          password: security?.userPassAuth?.password
            ?? env().SPEAKEASY_PASSWORD,
        },
      },
    ],
    [
      {
        ...
        type: "http:bearer",
        value: security?.option2?.bearerAuth ?? env().SPEAKEASY_BEARER_AUTH,
      },
      {
        fieldName: "api_key",
        type: "apiKey:header",
        value: security?.option2?.myApiKey ?? env().SPEAKEASY_MY_API_KEY,
      },
    ],
    [
      {
        fieldName: "Authorization",
        type: "oauth2",
        value: security?.option3?.oauth2 ?? env().SPEAKEASY_OAUTH2,
      },
    ],
    [
      {
        fieldName: "appId",
        type: "http:custom",
        value: security?.option4?.appId ?? env().SPEAKEASY_APP_ID,
      },
      {
        fieldName: "secret",
        type: "http:custom",
        value: security?.option4?.secret ?? env().SPEAKEASY_SECRET,
      },
    ],
    [
      {
        fieldName: "Authorization",
        type: "oauth2",
        value: security?.option5?.mobileAuth ?? env().SPEAKEASY_MOBILE_AUTH,
      },
    ],
    [
      {
        fieldName: "clientID",
        type: "oauth2:client_credentials",
        value: security?.option6?.clientID ?? env().SPEAKEASY_CLIENT_ID,
      },
      {
        fieldName: "clientSecret",
        type: "oauth2:client_credentials",
        value: security?.option6?.clientSecret ?? env().SPEAKEASY_CLIENT_SECRET,
      },
    ],
    [
      {
        fieldName: "api_key",
        type: "apiKey:header",
        value: security?.myApiKey?.myApiKey ?? env().SPEAKEASY_MY_API_KEY,
      },
    ],
  ];
