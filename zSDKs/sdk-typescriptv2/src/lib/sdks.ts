...
export class ClientSDK {
  readonly __httpClient: HTTPClient;
  readonly __hooks: SDKHooks;
  readonly __logger?: Logger | undefined;
  ...
  constructor(options: SDKOptions = {}) {
    ...
    if (
      ...
      this.__hooks = opt.hooks;
    } else {
      this.__hooks = new SDKHooks();
    }
    const defaultHttpClient = new HTTPClient();
    options.httpClient = options.httpClient || defaultHttpClient;
    options = this.__hooks.sdkInit(options);
    ...
    this._baseURL = url;
    this.__httpClient = options.httpClient || defaultHttpClient;

    this._options = { ...fillGlobals(options), hooks: this.__hooks };

    this.__logger = this._options.debugLogger;
    if (!this.__logger && env().SPEAKEASY_DEBUG) {
  ...
  public _createRequest(
    ...
    let input;
    try {
      input = this.__hooks.beforeCreateRequest(context, {
        url: reqURL,
        options: {
          ...fetchOptions,
          body: reqBody ?? null,
  ...
  > {
    ...
    return retry(
      async () => {
        ...
        const req = await this.__hooks.beforeRequest(context, attempt);
        await logRequest(this.__logger, req).catch((e) =>
          this.__logger?.log("Failed to log request:", e)
        );

        let response = await this.__httpClient.request(req);

        try {
          if (isErrorStatusCode(response.status)) {
            const result = await this.__hooks.afterError(
              context,
              response,
              null,
            );
            ...
            response = await this.__hooks.afterSuccess(context, response);
          }
        } finally {
          await logResponse(this.__logger, response, req)
            .catch(e => this.__logger?.log("Failed to log response:", e));
        }
  ...
  async _verifyWebhook(
    ...
    return this.__hooks.verifyWebhook({}, { request, secret });
  }
}
