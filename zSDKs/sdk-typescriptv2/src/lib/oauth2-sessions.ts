...
export class OAuth2SessionStore {
  ...
  getSession(
    ...
    const scopeKey = this.__getScopesKey(scopes);

    // First look for an exact match
    const exactMatch = tokens.get(scopeKey);
    if (
      exactMatch !== undefined && !this.__hasTokenExpired(exactMatch.expiresAt)
    ) {
      return exactMatch;
    }
    ...
    for (const [_, session] of tokens) {
      if (
        this.__hasRequiredScopes(session.scopes, scopes)
        && !this.__hasTokenExpired(session.expiresAt)
      ) {
        return session;
      }
    }
  ...
  storeSession(
    ...
    tokens.set(this.__getScopesKey(scopes), entry);
  }
  ...
  statistics(): {
    ...
    for (const [_, sessions] of this.cache) {
      for (const [_, session] of sessions) {
        ...
        if (this.__hasTokenExpired(session.expiresAt)) {
          expired += 1;
        } else {
          active += 1;
        }
  ...
  __getScopesKey(scopes: string[]): string {
    if (!scopes?.length) {
      return "";
    }
   ...
   */
  __hasTokenExpired(expiresAt: number | undefined): boolean {
