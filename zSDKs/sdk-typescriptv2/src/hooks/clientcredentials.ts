  ...
  implements SDKInitHook, BeforeRequestHook, AfterErrorHook
{
  ...
  private async getCredentialsGlobal(
    ...
    const DEFAULT_TOKEN_URL = "/clientcredentials/token";
    const envTokenURL = env().SPEAKEASY_TOKEN_URL ?? DEFAULT_TOKEN_URL;

    const additionalProperties: Record<string, string> = {};
    for (const [key, value] of Object.entries(out?.Option6 ?? {})) {
      if (
        typeof value === "string"
    ...
    return {
      clientID: out?.Option6?.clientID ?? env().SPEAKEASY_CLIENT_ID ?? "",
      clientSecret: out?.Option6?.clientSecret ?? env().SPEAKEASY_CLIENT_SECRET
        ?? "",
      tokenURL: out?.Option6?.tokenURL !== DEFAULT_TOKEN_URL
        ? out?.Option6?.tokenURL
        : envTokenURL,
      scopes: undefined,
