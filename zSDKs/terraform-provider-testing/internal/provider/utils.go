...
func debugResponse(response *http.Response) string {
	...
	if v := response.Request.Header.Get("X-Api-Key"); v != "" {
		response.Request.Header.Set("X-Api-Key", "(sensitive)")
	}
	if v := response.Request.Header.Get("X-Custom-Key"); v != "" {
		response.Request.Header.Set("X-Custom-Key", "(sensitive)")
	}
	if v := response.Request.Header.Get("X-Custom-Secret"); v != "" {
		response.Request.Header.Set("X-Custom-Secret", "(sensitive)")
	}
...
func fieldHeadersFromRequestReader(reader *textproto.Reader, fields map[string]interface{}) error {
	...
	if _, ok := fields["X-Api-Key"]; ok {
		fields["X-Api-Key"] = "(sensitive)"
	}
	if _, ok := fields["X-Custom-Key"]; ok {
		fields["X-Custom-Key"] = "(sensitive)"
	}
	if _, ok := fields["X-Custom-Secret"]; ok {
		fields["X-Custom-Secret"] = "(sensitive)"
	}
