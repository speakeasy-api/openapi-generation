...
variable "use_disc_string" {
  type = bool
}

variable "use_disc_number" {
  type = bool
}
...
resource "testing_oas_one_of" "test" {
  # Test discriminated union type switching between string and number variants
  optional_discriminator_request_and_response = var.use_disc_string ? {
    string = {
      nondiscriminator = "test"
      object_string = var.discriminator_value
    }
  } : (var.use_disc_number ? {
    number = {
      nondiscriminator = "test"
