...
class ClientCredentialsHook implements AfterErrorHook, BeforeRequestHook, SDKInitHook
{
    ...
    private function getCredentialsGlobal(\Closure $securitySource): ?Credentials
    {
        ...
        if (! $security instanceof \OpenAPI\OpenAPI\Security) {
            throw new \InvalidArgumentException('Invalid security type: expected \OpenAPI\OpenAPI\Security, got '.get_class($security));
        }

        if ($security->option6 == null || $security->option6->clientID == null || $security->option6->clientSecret == null || $security->option6->tokenURL == null) {
            return null;
        }
    ...
    private function doTokenRequest(string $baseUrl, Credentials $credentials, array $scopes): Session
    {
        ...
        try {
            $tokenResponse = $serializer->deserialize($responseBody, 'OpenAPI\OpenAPI\Hooks\TokenResponse', 'json', DeserializationContext::create()->setRequireAllRequiredProperties(true));
        } catch (\Exception $ex) {
