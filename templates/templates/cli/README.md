...
- **Eligibility**: non-`IsRequestBody` operations whose parameter maps to a top-level scalar flag (string, enum, int, number, date, date-time). Operations with two or more path parameters and promoted operations (exact-stutter operations whose `RunE` lives on the group command, where a positional would compete with subcommand names) stay flags-only.
...
- **Requiredness** (`ResolvePositionalFlag`, in the run function after the `--usage`/`--schema` short-circuits): the value is required once across both forms; neither form is `MissingRequiredFlagError` (`missing required flag: --<param> (or pass it as the [<param>] argument)`, exit 2). It is a no-op on commands without the annotation, so intent commands that reuse the run function are unaffected. The flag metadata keeps `Required: true`, so intent commands backed by the same operation keep their required flag.
- **Interactive mode**: when generated, the command declares an `interactive.ArgSpec` (`Required: true`, `SatisfiedBy: [<param>]`); a missing value prompts for the argument (`arg:<param>`), never the flag, and a supplied flag satisfies it.
- **`--usage`**: the static KDL node carries `arg "<param>" help="<summary>"` (not `required`, since the flag can satisfy it) and the rewritten flag help.
