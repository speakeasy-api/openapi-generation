...
- **Argument validation** (`PositionalFlagArgs`, before any pre-run): at most one argument; the argument is folded into the flag so request building reads a single source. Rejected: an argument together with the flag (`pass <param> once`), an argument starting with `-` (only reachable after `--`, so `op -- --dry-run` cannot become a live request with `--dry-run` as the identifier; `--<param>=-value` still works). A boolean-looking value (`true`, `false`) is a valid identifier and is accepted. Commands without the annotation keep `cobra.NoArgs` semantics.
- **Requiredness** (`ResolvePositionalFlag`, in the run function after the `--usage`/`--schema` short-circuits): a path parameter without a schema default is required once across both forms; neither form is `MissingRequiredFlagError` (`missing required flag: --<param> (or pass it as the [<param>] argument)`, exit 2). Parameters with a default remain optional. It is a no-op on commands without the annotation, so intent commands that reuse the run function are unaffected. The original flag metadata remains intact for intent commands backed by the same operation.
