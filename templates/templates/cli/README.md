...
Hint order is `CLILeadingHints()` (a local remediation that must lead, such as `--out` for a file path passed to `-o`), then server hints, then one taxonomy source (reason hints, declared type hints, or built-in type hints), then `CLIHints()`, then command-specific manifest hints. Exact duplicates are removed in first-occurrence order; malformed server hints never suppress local guidance. When a `CLI_VALIDATION` error has no leading hint, `flagutil.ShorthandConfusionHint` supplies one if the argv used a global shorthand (`-o`, `-q`, `-d`, `-H`) on a command with an unset, shorthand-less local flag starting with the same letter, and the error names that flag or the global flag (or a boolean shorthand spilled a value into positionals): `-q is short for --jq, not --query-string-param. Use --query-string-param qs`.
