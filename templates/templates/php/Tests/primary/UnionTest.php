...
use OpenAPI\OpenAPI\Models\Shared\CircularMultiLevelMiddle;
use OpenAPI\OpenAPI\Models\Shared\ConflictingDiscriminatorMappingKey;
use OpenAPI\OpenAPI\Models\Shared\PrefixTag2;
use OpenAPI\OpenAPI\Models\Shared\Tag;
use OpenAPI\OpenAPI\Models\Shared\TaggedObject1;
...
use OpenAPI\OpenAPI\Models\Shared\One;
use OpenAPI\OpenAPI\Models\Shared\UnionOfArrays2;
use OpenAPI\OpenAPI\Models\Shared\Three;
use OpenAPI\OpenAPI\Models\Shared\ArrayOfDiscriminatedUnionsMap;
use OpenAPI\OpenAPI\Models\Shared\NestedArrayOfDiscriminatedUnions;
...
use OpenAPI\OpenAPI\Models\Operations\OneOfCollectionEnumRes;
use OpenAPI\OpenAPI\Models\Shared\Nestedenum;
...
final class UnionTest extends TestCase
{
    ...
    public function testStronglyTypedOneOfPostDeep(): void
    {
        ...
        $this->assertNotNull($response);
        $this->assertEquals(200, $response->httpMeta->response->getStatusCode());
        $this->assertEquals(Helpers::endToEndObject($obj), $response->res->json);
    }
    ...
    public function testStronglyTypedOneOfDiscriminatedPost(): void
    {
        ...
        $obj = new TaggedObject1(imageURL: 'http://example.com/image.png', tag: Tag::Tag1);

        $response = $sdk->unions->stronglyTypedOneOfDiscriminatedPost(request: $obj);
        $this->assertNotNull($response);
        $this->assertEquals(200, $response->httpMeta->response->getStatusCode());
        $this->assertEquals($obj, $response->res->json);
    }

    // Guards the serialize path of UnionHandler against the two non-object/empty-array
    // inputs that previously threw a TypeError before any request was sent.
    public function testUnionSerializeGuards(): void
    {
        $serializer = JSON::createSerializer();

        // Discriminated union (explicit mapping) carried by a serialized field: the
        // serializer metadata is the [null, field, map] triple, which must not be walked.
        $discriminated = new ConflictingDiscriminatorMappingKey(
            prefix: new TaggedObject1(imageURL: 'http://example.com/image.png', tag: Tag::Tag1),
            prefixTag2: new PrefixTag2(str: 'test'),
        );
        $json = $serializer->serialize($discriminated, 'json');
        $this->assertStringContainsString('"tag":"tag1"', $json);
        $this->assertStringContainsString('image.png', $json);

        // Empty array held by a union-typed property: must serialize as [] rather than
        // reflecting over the value (get_class() on an array is fatal).
        $empty = new CircularMultiLevelMiddle(level3: []);
        $this->assertEquals('{"level3":[]}', $serializer->serialize($empty, 'json'));
    }
    ...
    public function testNullableOneOfTypeInObjectPost(): void
    {
        ...
        $tests = [
            [
                ...
                'want' => '{"OneOfOne":true,"NullableOneOfOne":null,"NullableOneOfTwo":null}'
            ],
            [
                ...
                'want' => '{"OneOfOne":true,"NullableOneOfOne":true,"NullableOneOfTwo":2}'
            ]
        ];
        ...
        foreach($tests as $test)
        {
            ...
            $this->assertEquals($test['want'], $req['body']);
    ...
    public function testNullableOneOfRefInObject(): void
    {
        ...
        $tests = [
            [
                ...
                'want' => '{"OneOfOne":{"type":"obj1","value":"one"},"NullableOneOfOne":null,"NullableOneOfTwo":null}'
            ],
            [
                'name' => 'Nullable fields set to null',
                'obj' => new NullableOneOfRefInObject(
            ...
            [
                ...
                'want' => '{"OneOfOne":{"type":"obj1","value":""},"NullableOneOfOne":{"type":"obj1","value":"one"},"NullableOneOfTwo":{"type":"obj2","value":"two"}}'
            ]
        ];
    ...
    public function testDateTimeBigintUnion() {
        ...
        $bigIntVal = \Brick\Math\BigInteger::of('9007199254740991');
        $response = $sdk->unions->unionDateTimeBigInt(request: $bigIntVal);
        $this->assertNotNull($response);
        $this->assertEquals(200, $response->httpMeta->response->getStatusCode());
        $this->assertEquals($bigIntVal, $response->res->json);
    }
    ...
    public function testUnionOfArrays() {
        ...
        $arrayOf1 = [new One('foo0'), new One('foo1')];
        $response = $sdk->unions->unionOfArraysPost(request: $arrayOf1);
        $this->assertNotNull($response);
        $this->assertEquals(200, $response->httpMeta->response->getStatusCode());
        $this->assertEquals($arrayOf1, $response->res->json);

        $arrayOf2 = [new UnionOfArrays2('bar0'), new UnionOfArrays2('bar1')];
        $response = $sdk->unions->unionOfArraysPost(request: $arrayOf2);
        $this->assertNotNull($response);
        $this->assertEquals(200, $response->httpMeta->response->getStatusCode());
        $this->assertEquals($arrayOf2, $response->res->json);

        $arrayOf3 = [new Three('baz0'), new Three('baz1')];
        $response = $sdk->unions->unionOfArraysPost(request: $arrayOf3);
        $this->assertNotNull($response);
        $this->assertEquals(200, $response->httpMeta->response->getStatusCode());
        $this->assertEquals($arrayOf3, $response->res->json);
    }

    public function testArrayOfDiscriminatedUnions() {
        ...
        $req = [Helpers::createSimpleObjectWithType(), Helpers::createDeepObjectWithType()];

        $response = $sdk->unions->arrayOfDiscriminatedUnions(request: $req);
        $this->assertNotNull($response);
        $this->assertEquals(200, $response->httpMeta->response->getStatusCode());
        $this->assertEquals(Helpers::endToEndObject($req[0]), $response->res->json[0]);
        $this->assertEquals(Helpers::endToEndObject($req[1]), $response->res->json[1]);
        $this->assertCount(2, $response->res->json);
        $this->assertEquals('simpleObjectWithType', $response->res->json[0]->type);
        $this->assertEquals('deepObjectWithType', $response->res->json[1]->type);
    }

    public function testArrayOfDiscriminatedUnionsMap() {
        ...
        $req = new ArrayOfDiscriminatedUnionsMap(['item' => [Helpers::createSimpleObjectWithType(), Helpers::createDeepObjectWithType()]]);

        $response = $sdk->unions->arrayOfDiscriminatedUnionsMap(request: $req);
        $this->assertNotNull($response);
        $this->assertEquals(200, $response->httpMeta->response->getStatusCode());
        $this->assertEquals(Helpers::endToEndObject($req->arrayMap['item'][0]), $response->res->json->arrayMap['item'][0]);
        $this->assertEquals(Helpers::endToEndObject($req->arrayMap['item'][1]), $response->res->json->arrayMap['item'][1]);
        $this->assertCount(2, $response->res->json->arrayMap['item']);
        $this->assertEquals('simpleObjectWithType', $response->res->json->arrayMap['item'][0]->type);
        $this->assertEquals('deepObjectWithType', $response->res->json->arrayMap['item'][1]->type);
    }

    public function testNestedArrayOfDiscriminatedUnions() {
        ...
        $simpleObject = Helpers::createSimpleObjectWithType();
        $deepObject = Helpers::createDeepObjectWithType();

        $req = new NestedArrayOfDiscriminatedUnions([[$simpleObject], [$simpleObject, $deepObject]]);

        $response = $sdk->unions->nestedArrayOfDiscriminatedUnions(request: $req);
        $this->assertNotNull($response);
        $this->assertEquals(200, $response->httpMeta->response->getStatusCode());
        $this->assertEquals(Helpers::endToEndObject($req), $response->res->json);
        $this->assertCount(2, $response->res->json->nestedArray);
        $this->assertCount(1, $response->res->json->nestedArray[0]);
        $this->assertEquals('simpleObjectWithType', $response->res->json->nestedArray[0][0]->type);
        $this->assertCount(2, $response->res->json->nestedArray[1]);
        $this->assertEquals('simpleObjectWithType', $response->res->json->nestedArray[1][0]->type);
        $this->assertEquals('deepObjectWithType', $response->res->json->nestedArray[1][1]->type);
    }


    public function testConstDiscriminator(): void
    {
        CommonHelpers::recordTest('unions-const-discriminator');
        $sdk = \OpenAPI\OpenAPI\SDK::builder()->build();
        $obj = new \OpenAPI\OpenAPI\Models\Shared\ConstObject1(imageURL: 'http://boo');
        $response = $sdk->unions->constDiscriminatedOneOf(request: $obj);
        $this->assertNotNull($response);
        $this->assertEquals(200, $response->httpMeta->response->getStatusCode());
        $this->assertEquals($obj, $response->res->json);
    }
    ...
    public function testBigIntStrDecimalUnion() {
        ...
        $response = $sdk->unions->unionBigIntStrDecimal(request: BigDecimal::of('3.14159265358979344719667586'));
        $this->assertNotNull($response);
        $this->assertEquals(200, $response->httpMeta->response->getStatusCode());
        $this->assertEquals(BigDecimal::of('3.1415926535898'), $response->res->json);

        $response = $sdk->unions->unionBigIntStrDecimal(request: BigInteger::of(9223372036854775807));
        $this->assertNotNull($response);
        $this->assertEquals(200, $response->httpMeta->response->getStatusCode());
        $this->assertEquals(BigInteger::of(9223372036854775807), $response->res->json);
    }

    public function testEnumNestedInArrayInUnion() {
        CommonHelpers::recordTest('union-enum-nested-in-array-in-union');
        $sdk = \OpenAPI\OpenAPI\SDK::builder()->setServerUrl(Helpers::getHttpBinUrl())->build();

        $this->assertInstanceOf(\OpenAPI\OpenAPI\SDK::class, $sdk);

        $val = new OneOfCollectionEnumRes(json: [Nestedenum::Abc]);

        $s = JSON::createSerializer();
        $dump = $s->serialize($val, 'json');
        $this->assertEquals($dump, '{"json":["abc"]}');
    }

    /**
     * Test deserialization of enum arrays within union types.
     * This validates the fix in UnionHandler::resolveArrayTypes() that correctly
     * resolves enum class names instead of using the literal string 'enum'.
     */
    public function testEnumArrayInUnionDeserialization(): void
    {
        CommonHelpers::recordTest('union-enum-array-deserialization');

        $s = JSON::createSerializer();

        // Test deserializing an array of enums within a union type
        // Before the fix, this would throw: ReflectionException: Class "\enum" does not exist
        $json = '{"json":["abc","def"]}';
        $result = $s->deserialize($json, OneOfCollectionEnumRes::class, 'json');

        $this->assertInstanceOf(OneOfCollectionEnumRes::class, $result);
        $this->assertIsArray($result->json);
        $this->assertCount(2, $result->json);
        $this->assertEquals(Nestedenum::Abc, $result->json[0]);
        $this->assertEquals(Nestedenum::Def, $result->json[1]);

        // Test round-trip: serialize and deserialize
        $original = new OneOfCollectionEnumRes(json: [Nestedenum::Abc, Nestedenum::Def]);
        $serialized = $s->serialize($original, 'json');
        $deserialized = $s->deserialize($serialized, OneOfCollectionEnumRes::class, 'json');

        $this->assertEquals($original->json, $deserialized->json);
    }
