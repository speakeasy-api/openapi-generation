...
type CharEncoding = "percent" | "none";
...
function encodingGroupForField(ann: ParamAnnotation): EncodingGroup | null {
  ...
  const charEncoding =
    type === "queryParam" || type === "pathParam" ? "percent" : "none";
...
function generateParamsSpec(options: {
  ...
  locals.forEach((fieldDef) => {
    ...
    const outboundName = sanitizeFieldName(originalFieldName(fieldDef));
    ...
    groups[target][ann.Name] = { name: ann.Name, accessor, fieldDef };
  });
  ...
  globals.forEach((fieldDef) => {
    ...
    if (!existing) {
      groups[target][name] = { name: ann.Name, accessor, fieldDef };
      return;
    }

    // Operation-level parameters take precedence over global ones
...
function genQueryParamEncoder(
  ...
  const groupings: Partial<Record<EncodingGroup, [string, string][]>> = {};

  for (const [_, { accessor, fieldDef }] of Object.entries(params)) {
    const ann = fieldDef.Annotations.Get("param");
    ...
    // For const fields, use the const value directly instead of accessor
    const valueExpr = fieldDef.Const
      ? JSON.stringify(fieldDef.Const.Value)
      : accessor;
    group.push([ann.Name, valueExpr]);
  }
  ...
  for (const [groupKey, fields] of sortedGroups) {
    ...
    if (charEncoding === "none") {
      opts.push(`charEncoding: "none"`);
    }
...
function genSingleParamEncoder(
  ...
  const explode = ann.Explode;
  const charEncoding = type === "pathParam" ? "percent" : "none";
