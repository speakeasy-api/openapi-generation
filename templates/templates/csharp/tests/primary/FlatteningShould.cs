...
public class FlatteningShould
{
    ...
    [Fact]
    public async Task RequiredBodyAllOptional()
    {
        CommonHelpers.RecordTest("flattening-required-body-all-optional");

        var sdk = new SDK(serverUrl: Helpers.HttpBinUrl);

        var res = await sdk.Flattening.RequiredBodyAllOptionalAsync(new ObjWithOptionalProperties()
        {
            OptStr = "body test",
            OptInt = 1,
        });
        Assert.Equal(HttpStatusCode.OK, res.HttpMeta.Response.StatusCode);
        Assert.Equal("body test", res.Res.Json.OptStr);
        Assert.Equal(1, res.Res.Json.OptInt);
    }
