...
public class RetriesShould
{
    ...
    public async Task RetriesTimeout()
    {
        ...
        Assert.Equal("API error occurred. Body: request failed please retry.", error.Message);
    ...
    public async Task GlobalRetryConfigDisable()
    {
        ...
        var sdk = new SDK(serverUrl: Helpers.HttpBinUrl, retryConfig: new RetryConfig(strategy: RetryConfig.RetryStrategy.NONE));
    ...
    [Fact]
    public async Task AttemptCountBackoff()
    {
        CommonHelpers.RecordTest("retries-attempt-count-backoff");

        var sdk = new SDK(serverUrl: Helpers.HttpBinUrl);

        var res = await sdk.Retries.RetriesAttemptCountAsync(
            requestId: RequestID(),
            numRetries: 3L,
            retryAfterMsVal: 1L
        );
    ...
    [Fact]
    public async Task AttemptCountBackoffZero()
    {
        ...
        var error = await Assert.ThrowsAsync<APIException>(
            async () =>
                await sdk.Retries.RetriesAttemptCountZeroAsync(
                    requestId: RequestID(),
                    numRetries: 2L,
                    retryAfterMsVal: 1L
                )
