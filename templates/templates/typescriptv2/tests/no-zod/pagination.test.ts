...
test("Test Pagination LimitOffset Offset Params", async () => {
  ...
  // No-zod: signature is (offset, limit) — spec parameter order is preserved
  // without zod's outbound remap, so positional args differ from primary.
  const res = await sdk.pagination.paginationLimitOffsetOffsetParams(0, limit);
  expect(res.httpMeta.response.status).toBe(200);
  expect(res.res).toBeDefined();
  expect(res.res!.resultArray.length).toBe(limit);
...
// Skipped request-side `default:` injection unimplemented.
// Without zod's outboundSchema.parse, `offset` ships as undefined.
test.skip("Test Pagination LimitOffset Default Offset Body", async () => {
...
test.skip("Test Pagination LimitOffset Default Offset Params", async () => {
...
// Skipped: request-side `default:` injection unimplemented in no-zod.
// Without zod's outboundSchema.parse, paginator's default page/offset
// ships as undefined and the retry loop never advances.
test.skip("Test Pagination With Retries", async () => {
...
test("Test Pagination Body Wrapped Request", async () => {
  ...
  const res = await sdk.pagination.paginationBodyWrappedRequest({
    // No-zod keeps the spec-driven body wrapper key (`body`) verbatim instead
    // of inlining the named field used by primary.
    body: {
      limit,
      page: 1,
    },
...
test("supports async iterators", async () => {
  ...
  // No-zod signature is (offset, limit); pass undefined for offset to use
  // the default and 3 for the per-page limit.
  const iterator = await sdk.pagination.paginationLimitOffsetOffsetParams(
    undefined,
    3,
  );
...
test("Test Pagination Cursor Nullable", async () => {
  ...
  // No-zod preserves the response-body wrapper key in its original Pascal
  // casing (PaginationCursorNullableLimitNextCursor) because there is no
  // outbound remap step to camelize it.
