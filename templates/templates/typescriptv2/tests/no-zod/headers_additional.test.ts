...
test("Headers Empty Response Body With Headers", async () => {
  ...
  // In the no-zod variant the headers dictionary is still emitted as a runtime
  // field alongside httpMeta, so we just sanity check it is reachable.
...
test("Headers Response Without Headers", async () => {
  ...
  const res200 = await s.responseHeaders.errorResponseHeaders(200, true);
...
test("Headers Error Responses Without Headers", async () => {
  ...
  await s.responseHeaders.responseHeaders(400, true).then(
    () => {
  ...
  // operation includes another response that has headers.
  await s.responseHeaders.responseHeaders(500, true).then(
    () => {
      expect.unreachable("expected APIError");
    },
    (err) => {
...
test("Headers Response Headers Optional", async () => {
  ...
  const res1 = await s.responseHeaders.responseHeaders(200, true);
  ...
  const res2 = await s.responseHeaders.responseHeaders(200, false);
...
test("Headers Error Response Headers Optional", async () => {
  ...
  await s.responseHeaders.errorResponseHeaders(429, true).then(
    () => {
      expect.unreachable("expected APIError");
    },
    (err1) => {
  ...
  await s.responseHeaders.errorResponseHeaders(429, false).then(
    () => {
      expect.unreachable("expected APIError");
    },
    (err2) => {
