...
test("CustomClient ProxyAgent", async () => {
  ...
  // In no-zod the response is the raw echoed httpbin payload. We expect
  // the string we sent to round-trip somewhere in the JSON payload.
  expect(JSON.stringify(res.string)).toContain("proxied");
});
