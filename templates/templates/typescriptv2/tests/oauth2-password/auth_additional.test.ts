...
test("uses oauth2 password credentials to obtain token and make request", async () => {
  ...
  expect(reqSpy).toHaveBeenCalledWith(
    "POST",
    `${API_TEST_SERVICE_URL}/oauth2/token`,
    {
      grant_type: "password",
      username: "testuser",
      password: "testpassword",
  ...
  expect(reqSpy).toHaveBeenCalledWith(
    "GET",
    `${API_TEST_SERVICE_URL}/ecommerce/products`,
    NOT_CAPTURED,
  );
...
test("bypasses oauth2 password hook with oauth2 access token", async () => {
  ...
  const tokenRes = await fetch(`${API_TEST_SERVICE_URL}/oauth2/token`, {
    method: "POST",
    headers: {
...
test("oauth2 password automatically obtains token with operation scope", async () => {
  ...
  // ========================================================================
  // createProduct should request a token with the "products:create" scope
  // ========================================================================
  ...
  // ================================================================================
  // The updateProduct operation requires both "products:read" and "products:create"
  // scopes so we expect another token to be requested.
  // ================================================================================
  const updateResult = await sdk.products.update({
    id: "123",
    productUpdateForm: { price: 21.49 },
  });
  expect(updateResult.httpMeta.response.status).toEqual(200);
  expect(updateResult.httpMeta.response.headers.get("x-oauth2")).toEqual(
    "pass",
  );
  expect(reqSpy).toHaveBeenCalledTimes(2);
  expect(reqSpy).toHaveBeenCalledWith(
  ...
  expect(reqSpy).toHaveBeenCalledWith(
    "PUT",
    `${API_TEST_SERVICE_URL}/ecommerce/products/123`,
    NOT_CAPTURED,
  );
  expect(updateResult.product?.price).toEqual(21.49);

  reqSpy.mockReset();

  // ===================================================================
  // The listProduct operation only requires the "products:read" scope
  // so we should reuse the token retrieved during the last call.
  // ===================================================================
...
test("oauth2 password fails to obtain token with bad credentials", async () => {
  ...
  try {
    ...
    expect(err.cause.message).toEqual(
      `Unexpected response for OAuth2 token request: Status 400. Body: {
  "error": "invalid_grant",
...
test("skips oauth2 flow for methods that do not require it", async () => {
  ...
  expect(reqSpy).toHaveBeenCalledWith(
    "GET",
    `${API_TEST_SERVICE_URL}/ping`,
    NOT_CAPTURED,
  );
  expect(result.res).toEqual("pong");
});

test("oauth2 password flow as operation-level security option", async () => {
  ...
  const res = await sdk.inventory.updateProductStock(
    {
      oauth2: {
        clientID: "beezy",
        clientSecret: "super-secret",
    ...
    {
      productInventoryUpdateForm: { quantityDelta: 99 },
      id: "123",
    },
  );

  expect(res.httpMeta.response.status).toEqual(200);
  expect(res.httpMeta.response.headers.get("x-oauth2")).toEqual("pass");
  expect(reqSpy).toHaveBeenCalledTimes(2);
  expect(reqSpy).toHaveBeenCalledWith(
    "POST",
    `${API_TEST_SERVICE_URL}/oauth2/token`,
  ...
  expect(reqSpy).toHaveBeenCalledWith(
    "PUT",
    `${API_TEST_SERVICE_URL}/ecommerce/products/123/inventory`,
    NOT_CAPTURED,
  );

  expect(res.productInventoryStatus).toBeDefined();
  expect(res.productInventoryStatus!.productId).toEqual("123");
  expect(res.productInventoryStatus!.quantity).toBeGreaterThanOrEqual(99);
});
