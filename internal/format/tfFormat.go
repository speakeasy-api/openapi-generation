...
//
// This file contains a modified and relocated subset of
// https://github.com/hashicorp/terraform/blob/55792309eb6320d38ac6cad52eb379d0dd745366/internal/command/fmt.go.
// The MPL-2.0 notice applies to the copied material; see NOTICE for source and
// redistribution details, and https://mozilla.org/MPL/2.0/ for the license.
...
// tfFormat is a Command implementation that rewrites Terraform config
// files to a canonical format and style.
type tfFormat struct{}

func (c *tfFormat) MatchesSuffix(path string) bool {
	for _, ext := range tfFmtSupportedExts {
...
func (c *tfFormat) formatSourceCode(src []byte, filename string) []byte {
...
func (c *tfFormat) formatBody(body *hclwrite.Body, inBlocks []string) {
...
func (c *tfFormat) formatValueExpr(tokens hclwrite.Tokens) hclwrite.Tokens {
	if len(tokens) < 5 {
...
func (c *tfFormat) formatTypeExpr(tokens hclwrite.Tokens) hclwrite.Tokens {
	switch len(tokens) {
	case 1:
		kwTok := tokens[0]
...
func (c *tfFormat) trimNewlines(tokens hclwrite.Tokens) hclwrite.Tokens {
	if len(tokens) == 0 {
		return nil
	}
	var start, end int
